<template xmlns="http://www.w3.org/1999/html">
  <div style="width: 80%; margin: 0px auto">
    ...
    <div style="justify-content: center; float: left">
      <h1></h1>
      <div class="leftFloat" v-for="item in image1" :key="item.url">
        <img :src="item.url" style="height: 60%; width: 60%" alt="" />
      </div>
      <div class="rightFloat">
        <h2>可回收垃圾(RECYLABLE)</h2>
        <h3>
          可回收物指适宜回收利用和资源化利用的生活废弃物。可回收物主要品种包括：废纸、废弃塑料瓶、废金属、废包装物、废旧纺织物、废弃电器电子产品、废玻璃、废纸塑铝复合包装等。
        </h3>
        <p>
          根据《城市生活垃圾分类及其评价标准》行业标准以及参考德国垃圾分类法，可回收物是指适宜回收循环使用和资源利用的废物。主要包括：1、纸类：未严重玷污的文字用纸、包装用纸和其他纸制品等。如报纸、各种包装纸、办公用纸、广告纸片、纸盒、复印纸等；2、塑料：废容器塑料、包装塑料等塑料制品。比如各种塑料袋、塑料瓶、泡沫塑料、一次性塑料餐盒餐具、硬塑料等；3、金属：各种类别的废金属物品。如易拉罐、铁皮罐头盒、铅皮牙膏皮等；4、玻璃：有色和无色废玻璃制品；5、织物：旧纺织衣物和纺织制品。有害垃圾是指油漆、颜料、各类清洗液、电子垃圾和电池。
        </p>
    ...
    <div style="justify-content: center; float: left">
      <h1></h1>
      <div class="leftFloat" v-for="item in image2" :key="item.url">
        <img :src="item.url" style="height: 60%; width: 60%" alt="" />
      </div>
      <div class="rightFloat">
        <h2>有害垃圾(HAZARDOUS)</h2>
        <h3>
          有害垃圾指对人体健康或者自然环境造成直接或者潜在危害的生活废弃物。常见的有害垃圾包括废灯管、废油漆、杀虫剂、废弃化妆品、过期药品、废电池、废灯泡、废水银温度计等，有害垃圾需按照特殊正确的方法安全处理
        </h3>
        <p>
          1、废镍镉电池和氧化汞电池（包括手机
          、iPad、照相机等使用的充电电池以及纽扣电池）以及单子类危险废物；日光灯、节能灯等废荧光灯类、灯泡；
          废药品及其包装物、废杀虫剂和消毒剂及其包装物、废水银温度计、废水银血压计、废胶片及废相纸、废硒鼓墨盒；
      ...
      <el-popover placement="right" width="400" trigger="click">
        <el-table :data="Detail" height="280px" stripe>
          <el-table-column width="360px" prop="name" label="有毒垃圾目录">
          </el-table-column>
        </el-table>
        <el-button slot="reference" type="danger" @click="getHazardousBarbage()"
          >查看详情</el-button
        >
      </el-popover>
    </div>
    <div style="justify-content: center; float: left">
      <h1></h1>
      <div class="leftFloat" v-for="item in image3" :key="item.url">
        <img :src="item.url" style="height: 60%; width: 60%" alt="" />
      </div>
      <div class="rightFloat">
        <h2>湿垃圾(HOUSEHOLD)</h2>
        <h3>
          湿垃圾（英文名称也称：organic
          waste）又称为厨余垃圾、有机垃圾，即易腐垃圾，指食材废料、剩菜剩饭、过期食品、瓜皮果核、花卉绿植、中药药渣等易腐的生物质生活废弃物。
        </h3>
        <p>
          湿垃圾是指日常生活垃圾中可分解的有机物质部分。有机垃圾包括食物残渣、菜根、菜叶，动物蹄、角、瓜皮、果屑、蛋壳、鱼鳞、毛发、植物枝干、树叶、杂草、动物尸体、牲畜粪便等。有机垃圾应由专人负责收
          集，收集容器必须密封、有盖、防渗漏、防蝇、防鼠。
          湿垃圾是居民日常生活及食品加工、饮食服务、单位供餐等活动中产生的垃圾，包括丢弃不用的菜叶、剩菜、剩饭、果皮、蛋壳、茶渣、骨头、动物内脏、鱼鳞、树叶、杂草等。
        </p>
        <el-divider></el-divider>
      </div>
      <el-popover placement="right" width="400" trigger="click">
        <el-table :data="Detail" height="280px" stripe>
          <el-table-column width="360px" prop="name" label="湿垃圾目录">
          </el-table-column>
        </el-table>
        <el-button
          slot="reference"
    ...
    <div style="justify-content: center; float: left">
      <h1></h1>
      <div class="leftFloat" v-for="item in image4" :key="item.url">
        <img :src="item.url" style="height: 60%; width: 60%" alt="" />
      </div>
      <div class="rightFloat">
        <h2>干垃圾(RESIDUAL)</h2>
        ...
        <p>
          干垃圾包括：餐巾纸、卫生间用纸、尿不湿、猫砂、狗尿垫、污损纸张、烟蒂、干燥剂，污损塑料、尼龙制品、编织袋、防碎气泡膜，
          大骨头、硬贝壳、硬果壳（椰子壳、榴莲壳、核桃壳、玉米衣、甘蔗皮）、硬果实（榴莲核、菠萝蜜核），毛发、灰土、炉渣、橡皮泥、太空沙、带胶制品，
          一次性餐具、镜子、竹制品，成分复杂的制品。
          危害较小，但无再次利用价值。
        </p>
        <el-divider></el-divider>
      </div>
      ...
      <el-popover placement="right" width="400" trigger="click">
        <el-table :data="Detail" height="280px" stripe>
          <el-table-column width="360px" prop="name" label="干垃圾目录">
          </el-table-column>
        </el-table>
        <el-button slot="reference" type="info" @click="getResidualBarbage()"
          >查看详情</el-button
        >
      </el-popover>
    </div>
    ...
    <div style="justify-content: center; float: left">
      <h3>图片识别垃圾分类</h3>
      <div class="leftFloat" style="width: auto; height: auto">
        <el-upload
          ref="upload"
      ...
      <h3
        style="float: left; margin-top: 22px; margin-left: 20px"
